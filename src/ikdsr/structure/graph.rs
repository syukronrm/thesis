...
impl Graph {
    fn node_indices(&self) -> NodeIndices<Node> {
        self.inner.node_indices()
    }

    fn find_edge(&self, a: NodeIndex, b: NodeIndex) -> EdgeIndex {
        if let Some(e) = self.inner.find_edge(a, b) {
            e
        } else {
            panic!("Nodes is not connected!");
        }
    }

    fn neighbors(&self, node: NodeIndex) -> Neighbors<Edge> {
        self.inner.neighbors(node)
    }

    fn edge_len(&self, edge: EdgeIndex) -> f32 {
